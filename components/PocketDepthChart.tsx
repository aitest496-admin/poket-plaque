...
const PocketDepthChart: React.FC<PocketDepthChartProps> = ({ values, onChange, isInverted = false, singlePoint = false, displayPoints }) => {
  ...
  const inputHeight = isLarge ? 'h-[42px]' : 'h-[39px]';
  const cellHeight = isLarge ? 'h-[22px]' : 'h-[19px]';
  const inputTextSize = isLarge ? 'text-[18px]' : 'text-[14px]';
  const cellTextSize = isLarge ? 'text-[14px]' : 'text-[10px]';
